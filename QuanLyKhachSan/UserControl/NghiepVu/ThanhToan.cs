...
namespace QuanLyKhachSan.UserControl.NghiepVu
{
    ...
    public partial class ThanhToan : DevExpress.XtraEditors.XtraUserControl
    {
            ...
            try {
                if(func.isNumber(txbSDT.Text))
                {
                    ...
                    if (txbSDT.Text.Length != 0)
                    {
                        ...
                        sqlReader1.Close();
                        SqlCommand cmd7 = new SqlCommand("SELECT SUM(GiaDichVu*SuDungDichVu.SoLuong) as Gia FROM SuDungDichVu,DICHVU,KHACHHANG,HOADON WHERE SuDungDichVu.MaHoaDon=HOADON.MaHoaDon AND SuDungDichVu.MaDichVu = DICHVU.MaDichVu AND HOADON.MaKH = KHACHHANG.MaKH AND TinhTrangThanhToan = 0 AND HOADON.MaHoaDon = @mahd GROUP BY HOADON.MaHoaDon", DataAccessContext.connection);
                        ...
                        string cmd2 = "SELECT TENDV 'Tên dịch vụ',GiaDichVu 'Giá dịch vụ',SuDungDichVu.SoLuong 'Số lượng',SuDungDichVu.NgayDungDichVu 'Ngày dùng Dịch Vụ' FROM SuDungDichVu,DICHVU,KHACHHANG,HOADON WHERE SuDungDichVu.MaHoaDon=HOADON.MaHoaDon AND SuDungDichVu.MaDichVu = DICHVU.MaDichVu AND HOADON.MaKH = KHACHHANG.MaKH AND TinhTrangThanhToan = 0 AND HOADON.MaHoaDon =" + mahd;
                ...
                {
                    XtraMessageBox.Show("Số điện thoại phải là số từ 0 -> 9");
                }
        ...
        private void BtnThanhToan_Click(object sender, EventArgs e)
        {
            ...
            else { XtraMessageBox.Show("Thanh toán thất bại","",MessageBoxButtons.OK,MessageBoxIcon.Error); }
        }
    }
